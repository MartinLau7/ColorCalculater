...
#define ED_DEFAULT_BUTTON_HEIGHT	85.0
...
-(id)init
{
	...
	if( self )
	{
		...
		[self setFont:[NSFont fontWithName:@"Lucida Grande" size:13]];
        self.realTarget = nil;
        self.realAction = nil;
	}
	return self;
...
- (void)drawWithFrame:(NSRect)frame inView:(NSView *)view {
	...
	if( [self state] == NSOnState )
		cellColor = [NSColor colorWithRed:33/255.0 green:196/255.0 blue:109/255.0 alpha:.8];
	else
		cellColor = [NSColor colorWithRed:33/255.0 green:196/255.0 blue:109/255.0 alpha:1];
	...
	NSRect rectTitle = NSMakeRect(frame.origin.x, frame.origin.y+frame.size.height-30, frame.size.width, 20);
...
- (void)drawBezelWithFrame:(NSRect)frame inView:(NSView *)view {
	...
	if( [self state] == NSOnState )
		cellColor = [NSColor colorWithRed:33/255.0 green:196/255.0 blue:109/255.0 alpha:0.8];
	else
		cellColor = [NSColor colorWithRed:33/255.0 green:196/255.0 blue:109/255.0 alpha:1];
	...
	CGContextSetRGBFillColor(ctx, 33.0/255.0, 196.0/255.0, 109.0/255.0, 1.0);
	CGContextFillRect(ctx, CGRectMake(0.5, NSMaxY(frame)-2, frame.size.width-1, 1));
	CGContextSetRGBFillColor(ctx, 77.0/255.0, 196.0/255.0, 109.0/255.0, 1.0);
...
-(void)selectButtonAtRow:(NSUInteger)row
{
    ...
    if(self.currentSelectedIndex != rowToSelect){
        self.currentSelectedIndex = rowToSelect;
        //Target
...
#pragma clang diagnostic pop            
    ...
    if(self.currentSelectedIndex != row){
        if( sidebarDelegate && [sidebarDelegate respondsToSelector:@selector(sideBar:didSelectButton:)] ) {
            [sidebarDelegate sideBar:self didSelectButton:row];
        }
